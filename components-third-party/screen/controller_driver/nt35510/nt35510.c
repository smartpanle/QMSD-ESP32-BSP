...
scr_driver_t lcd_nt35510_default_driver = {
    ...
    .init_reg = NULL,
    .write_cmd = lcd_nt35510_write_cmd,
    .write_data = lcd_nt35510_write_data,
};
...
esp_err_t lcd_nt35510_init(const scr_controller_config_t *lcd_conf)
{
    ...
    if (lcd_nt35510_default_driver.init_reg) {
        ret = lcd_nt35510_default_driver.init_reg();
    } else {
...
esp_err_t lcd_nt35510_write_cmd(uint16_t cmd) {
    return LCD_WRITE_CMD_16B(cmd);
}

esp_err_t lcd_nt35510_write_data(uint16_t data) {
    return LCD_WRITE_DATA_16B(data);
}
