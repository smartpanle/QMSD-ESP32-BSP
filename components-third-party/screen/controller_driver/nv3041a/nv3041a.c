...
static const char *TAG = "lcd nv3041a";
...
#define LCD_NAME "NV3041A"
#define LCD_BPP  16

#define NV3041A_RESOLUTION_HOR 480
#define NV3041A_RESOLUTION_VER 272
...
scr_driver_t lcd_nv3041a_default_driver = {
    .init = lcd_nv3041a_init,
    .deinit = lcd_nv3041a_deinit,
    .set_direction = lcd_nv3041a_set_rotation,
    .set_window = lcd_nv3041a_set_window,
    .write_ram_data = lcd_nv3041a_write_ram_data,
    .draw_pixel = lcd_nv3041a_draw_pixel,
    .draw_bitmap = lcd_nv3041a_draw_bitmap,
    .get_info = lcd_nv3041a_get_info,
    .init_reg = NULL,
    .write_cmd = lcd_nv3041a_write_cmd,
    .write_data = lcd_nv3041a_write_data,
};

static esp_err_t lcd_nv3041a_reg_config(void);

esp_err_t lcd_nv3041a_init(const scr_controller_config_t *lcd_conf)
{
    LCD_CHECK(lcd_conf->width <= NV3041A_RESOLUTION_HOR, "Width greater than maximum", ESP_ERR_INVALID_ARG);
    LCD_CHECK(lcd_conf->height <= NV3041A_RESOLUTION_VER, "Height greater than maximum", ESP_ERR_INVALID_ARG);
    ...
    if (lcd_nv3041a_default_driver.init_reg) {
        ret = lcd_nv3041a_default_driver.init_reg();
    } else {
        // Send all the commands
        ret = lcd_nv3041a_reg_config();
    }
...
esp_err_t lcd_nv3041a_deinit(void)
{
    memset(&g_lcd_handle, 0, sizeof(scr_handle_t));
    return ESP_OK;
}

esp_err_t lcd_nv3041a_set_rotation(scr_dir_t dir)
{
    esp_err_t ret;
    uint8_t reg_data = 0;
    // reg_data |= MADCTL_RGB;
...
esp_err_t lcd_nv3041a_get_info(scr_info_t *info)
{
...
esp_err_t lcd_nv3041a_set_window(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1)
{
    ...
    scr_utility_apply_offset(&g_lcd_handle, NV3041A_RESOLUTION_HOR, NV3041A_RESOLUTION_VER, &x0, &y0, &x1, &y1);
...
esp_err_t lcd_nv3041a_write_ram_data(uint16_t color)
{
...
esp_err_t lcd_nv3041a_set_invert(bool is_invert)
{
    return LCD_WRITE_CMD(is_invert ? LCD_INVON : LCD_INVOFF);
}

esp_err_t lcd_nv3041a_draw_pixel(uint16_t x, uint16_t y, uint16_t color)
{
    esp_err_t ret;
    ret = lcd_nv3041a_set_window(x, y, x, y);
    if (ESP_OK != ret) {
        return ESP_FAIL;
    }
    return lcd_nv3041a_write_ram_data(color);
}

esp_err_t lcd_nv3041a_draw_bitmap(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t *bitmap)
{
    esp_err_t ret;
    LCD_CHECK(NULL != bitmap, "bitmap pointer invalid", ESP_ERR_INVALID_ARG);

    LCD_IFACE_ACQUIRE();
    ret = lcd_nv3041a_set_window(x, y, x + w - 1, y + h - 1);
    if (ESP_OK != ret) {
        return ESP_FAIL;
    }
...
esp_err_t lcd_nv3041a_write_cmd(uint16_t cmd) {
    return LCD_WRITE_CMD(cmd);
}

esp_err_t lcd_nv3041a_write_data(uint16_t data) {
