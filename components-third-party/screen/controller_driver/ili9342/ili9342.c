...
scr_driver_t lcd_ili9342_default_driver = {
    ...
    .init_reg = NULL,
    .write_cmd = lcd_ili9342_write_cmd,
    .write_data = lcd_ili9342_write_data,
};
...
esp_err_t lcd_ili9342_init(const scr_controller_config_t *lcd_conf)
{
    ...
    if (lcd_ili9342_default_driver.init_reg) {
        ret = lcd_ili9342_default_driver.init_reg();
    } else {
...
esp_err_t lcd_ili9342_write_cmd(uint16_t cmd) {
    return LCD_WRITE_CMD(cmd);
}

esp_err_t lcd_ili9342_write_data(uint16_t data) {
