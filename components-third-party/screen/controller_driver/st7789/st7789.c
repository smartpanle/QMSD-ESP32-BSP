...
esp_err_t lcd_st7789_init(const scr_controller_config_t *lcd_conf)
{
    ...
    if (lcd_st7789_default_driver.init_reg) {
        ret = lcd_st7789_default_driver.init_reg();
    } else {
...
esp_err_t lcd_st7789_write_cmd(uint16_t cmd) {
    return LCD_WRITE_CMD(cmd);
}

esp_err_t lcd_st7789_write_data(uint16_t data) {
