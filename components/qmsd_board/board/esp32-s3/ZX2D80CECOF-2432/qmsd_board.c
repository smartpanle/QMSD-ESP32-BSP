...
void qmsd_board_init(qmsd_board_config_t* config) {
    ...
    if (BOARD_RESET_PIN > -1) {
        ...
        gpio_hal_iomux_func_sel(GPIO_PIN_MUX_REG[TP_I2C_INT_PIN], PIN_FUNC_GPIO);
        gpio_set_direction(TP_I2C_INT_PIN, GPIO_MODE_OUTPUT);
        gpio_set_level(TP_I2C_INT_PIN, 0);
        ...
        gpio_set_direction(TP_I2C_INT_PIN, GPIO_MODE_DISABLE);
    }
