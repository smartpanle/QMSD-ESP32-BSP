...
qmsd_widget_t* qmsd_subscreen_create(qmsd_widget_t* parent);
 ...
 * @return int 
 */
int16_t qmsd_obj_get_x_ori(qmsd_widget_t* obj);
 ...
 * @param y 
 */
void qmsd_obj_set_pos_ori(qmsd_widget_t* obj,int x,int y);

/**
 * @brief set parent of widget
 * 
 * @param obj 
 * @param parent 
 */
void qmsd_widget_set_parent(qmsd_widget_t* obj,qmsd_widget_t* parent);
...
/**
 * @brief align widget to it's parent
 * 
 * @param obj obj to be aligned
 * @param align 
 * @param ofs_x extra x offset
 * @param osf_y extra y offset
 */
void qmsd_widget_align(qmsd_widget_t* obj,qmsd_align_t align,int16_t ofs_x,int16_t osf_y);

/**
 * @brief align widget to another widget
 * 
 * @param obj 
 * @param base 
 * @param align 
...
void __qmsd_##id##_cb(qmsd_widget_t* obj,qmsd_event_t event,void* user_data); \
static inline void qmsd_##id##_port_cb(lv_obj_t* obj , lv_event_t event) \
{ \
    __qmsd_##id##_cb(obj,event,NULL); \
} 

#define qmsd_widget_set_port_cb(id) lv_obj_set_event_cb(id.widget,qmsd_##id##_port_cb)
#define qmsd_widget_share_port_cb(widget,id) lv_obj_set_event_cb(widget,qmsd_##id##_port_cb)
...
{ \
    __qmsd_##id##_cb(event->target,event->code,event->user_data); \
} 

#define qmsd_widget_set_port_cb(id) lv_obj_add_event_cb(id.widget,qmsd_##id##_port_cb,LV_EVENT_ALL,id.ext_args)
#define qmsd_widget_share_port_cb(widget,id) lv_obj_add_event_cb(widget,qmsd_##id##_port_cb,LV_EVENT_ALL,NULL)
#define qmsd_widget_set_cb(id , cb , event , user_data) lv_obj_add_event_cb(id,cb,event,user_data)
